...
public class PrTraps : MonoBehaviour {
    ...
    void Start () {
        ...
        vehiclesGO = new List<GameObject>();
    ...
    void ApplyDamage()
    {
        ...
        foreach (GameObject e in vehiclesGO)
        {
            if (e != null)
            {
                e.SendMessage("ApplyDamagePassive", damage, SendMessageOptions.DontRequireReceiver);
                if (vfxPrefab)
                    PlayVFX(e.transform);
    ...
    void OnTriggerEnter(Collider other)
    {
        if (isActive)
        {
            ...
            if (other.gameObject.tag == "Vehicles" && affectVehicles)
            {
                damageOn = true;
                vehiclesGO.Add(other.gameObject);
                //ApplySpeed(other.gameObject, movSpeedFactor);
            }
        }
    ...
    void OnTriggerExit(Collider other)
    {
        if (isActive)
        {
            ...
            if (other.gameObject.tag == "Vehicles" && affectVehicles)
            {
                damageOn = false;
                //ApplySpeed(other.gameObject, 1.0f);
                vehiclesGO.Remove(other.gameObject);
