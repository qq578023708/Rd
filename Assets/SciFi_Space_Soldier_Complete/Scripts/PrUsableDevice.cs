...
public class PrUsableDevice : MonoBehaviour {
	...
	public void Use()
	{
        if (IsEnabled && !InUse)
        {
            ...
            {
                if (User.GetComponent<PrCharacterInventory>().BlueKeys > 0)
                {
                    User.GetComponent<PrCharacterInventory>().BlueKeys -= 1;
                    KeyType = Key.None;
                    InUse = true;
                }
                else if (User.GetComponent<PrCharacterInventory>().FullKeys > 0)
                {
                    User.GetComponent<PrCharacterInventory>().FullKeys -= 1;
                    KeyType = Key.None;
                    InUse = true;
                }
                else
            ...
            {
                if (User.GetComponent<PrCharacterInventory>().YellowKeys > 0)
                {
                    User.GetComponent<PrCharacterInventory>().YellowKeys -= 1;
                    KeyType = Key.None;
                    InUse = true;
                }
                else if (User.GetComponent<PrCharacterInventory>().FullKeys > 0)
            ...
            {
                if (User.GetComponent<PrCharacterInventory>().RedKeys > 0)
                {
                    User.GetComponent<PrCharacterInventory>().RedKeys -= 1;
                    KeyType = Key.None;
                    InUse = true;
                }
                else if (User.GetComponent<PrCharacterInventory>().FullKeys > 0)
    ...
    void OnTriggerStay(Collider other)
    {
        if (other.CompareTag("Player") && MeshSelector && IsEnabled)
        {
            ...
            {
                if (other.gameObject.GetComponent<PrCharacterInventory>().RedKeys > 0)
                {
                    UseText.text = "Use";
