...
public class PrBullet : MonoBehaviour {
    ...
    private PrActor finalTarget;
    [HideInInspector]
    public int playerNmb = 99;
	...
	void Update () {
        ...
        if (!UsePhysicsToTranslate)
        {
            ...
            transform.Translate(Vector3.forward * BulletSpeed * 30 * Time.deltaTime);
        ...
        RaycastHit hit;
        float maxDistance = Mathf.Clamp(BulletSpeed * 0.05f, 0.8f, Mathf.Infinity);
    ...
    void DestroyGrenade(Vector3 HitNormal, Vector3 HitPos, Transform HitTransform)
    {
        ...
        foreach (Collider col in colls)
        {
            ...
            {
                if (col.GetComponent<PrActorUtils>())
                {
                    if (!friendlyFire)
                    {
                        if (col.GetComponent<PrActorUtils>().team != team)
                        {
                            if (col.GetComponent<Rigidbody>())
                            {
                                col.gameObject.GetComponent<Rigidbody>().AddExplosionForce(RadialForce, explosivePos, DamageRadius * 3);
                            }
                            finalTarget = col.GetComponent<PrActorUtils>().character;
                            finalTarget.ApplyDamage(Damage, temperatureMod, team, transform.position, generatesBloodDamage, HitForce, playerNmb);
            ...
            else if (col.CompareTag("AIPlayer") && col.GetComponent<PrActorUtils>().team != team)
            {
                if (col.GetComponent<Rigidbody>())
                {
                    col.gameObject.GetComponent<Rigidbody>().AddExplosionForce(RadialForce, explosivePos, DamageRadius * 3);
            ...
            else if (col.CompareTag("Enemy") && col.GetComponent<PrActorUtils>().team != team)
            {
                if (col.GetComponent<Rigidbody>())
                {
                    col.gameObject.GetComponent<Rigidbody>().AddExplosionForce(RadialForce, explosivePos, DamageRadius * 3);
    ...
    void DestroyBullet(Vector3 HitNormal, Vector3 HitPos, Transform HitTransform, GameObject Target, string HitTag)
	{
        ...
        if (!RadialDamage)
        {
            ...
            else if (HitTag == "Player" || HitTag == "Enemy" || HitTag == "AIPlayer" || HitTag == "Vehicles")
            {
                if (Target.GetComponent<PrActorUtils>())
                {
                    if (Target.GetComponent<PrActorUtils>().team != team)
                    {
                        finalTarget = Target.GetComponent<PrActorUtils>().character;
                        finalTarget.ApplyDamage(Damage, temperatureMod, team, transform.position, generatesBloodDamage, HitForce, playerNmb);
                ...
                else if (Target.GetComponent<PrVehicle>())
                {
                    
                    if (Target.GetComponent<PrVehicle>().team != team)
                    {
                        finalTarget = Target.GetComponent<PrVehicle>();
                        finalTarget.ApplyDamage(Damage, temperatureMod, team, transform.position, generatesBloodDamage, HitForce, playerNmb);

                        ////Debug.Log("Damage Done to vehicle " + Damage + " " + finalTarget.ActualHealth);
            ...
            /*else if (HitTag == "AIPlayer" && Target.GetComponent<PrAIController>().team != team)
            {
                ...
                {
                    Target.SendMessage("ApplyDamage", Damage, SendMessageOptions.DontRequireReceiver);
                    if (Target.GetComponent<PrAIController>().damageVFX != null)
                    {
                        Instantiate(Target.GetComponent<PrAIController>().damageVFX, HitPos, Quaternion.LookRotation(HitNormal));
                        UseDefaultImpactFX = false;
                    }
                }
            ...
            /*
            else if (Target.GetComponent<PrEnemyAI>().team != team)
            {
                // //Debug.Log("Bullet team = " + team + " Target Team = " + Target.GetComponent<PrEnemyAI>().team);
                Target.SendMessage("PlayerTeam", team, SendMessageOptions.DontRequireReceiver);
                Target.SendMessage("BulletPos", transform.position, SendMessageOptions.DontRequireReceiver);
        ...
        else
        {
            ...
            foreach (Collider col in colls)
            {
                if (col != null)
                {
                    ...
                    else if (col.CompareTag("Player") || col.CompareTag("AIPlayer") || col.CompareTag("Enemy") || col.CompareTag("Vehicles"))
                    {
                        if (col.GetComponent<PrActorUtils>() && col.GetComponent<PrActorUtils>().team != team)
                        {
                            finalTarget = col.GetComponent<PrActorUtils>().character;
                            finalTarget.ApplyDamage(Damage, temperatureMod, team, transform.position, generatesBloodDamage, HitForce, playerNmb);
                            ...
                            if (generatesBloodDamage)
                            {
                                //finalTarget.SendMessage("ApplyDamage", Damage, SendMessageOptions.DontRequireReceiver);
                                if (finalTarget.DamageFX != null)
                                {
                                    Instantiate(finalTarget.DamageFX, HitPos, Quaternion.LookRotation(HitNormal));
                                    //UseDefaultImpactFX = false;
                        ...
                        else if (col.GetComponent<PrVehicle>())
                        {

                            if (col.GetComponent<PrVehicle>().team != team)
                            {
                                finalTarget = col.GetComponent<PrVehicle>();
                                finalTarget.ApplyDamage(Damage, temperatureMod, team, transform.position, generatesBloodDamage, HitForce, playerNmb);
                ...
                /*
                else if (col.CompareTag("AIPlayer") && col.GetComponent<PrAIController>().team != team)
                {
                    if (col.GetComponent<Rigidbody>())
                    {
                        col.gameObject.GetComponent<Rigidbody>().AddExplosionForce(RadialForce, explosivePos, DamageRadius * 3);
                ...
                else if (col.CompareTag("Enemy") && col.GetComponent<PrActorUtils>())
                {
                    if (col.GetComponent<Rigidbody>())
                    {
                        col.gameObject.GetComponent<Rigidbody>().AddExplosionForce(RadialForce, explosivePos, DamageRadius * 3);
