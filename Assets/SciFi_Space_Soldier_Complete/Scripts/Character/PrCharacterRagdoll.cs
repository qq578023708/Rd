...
public class PrCharacterRagdoll : MonoBehaviour {
    ...
    public void SetForceToRagdoll(Vector3 position, Vector3 force, Transform target)
    {
        ...
        if (target != null && target.GetComponent<Rigidbody>() != null)
        {
            ...
            //Debug.LogWarning("target Bone : " + targetBone);
        }
        else
        {
            //getClosestBone
            ...
            foreach (GameObject go in ragdollBones)
            {
                if (go != null)
                {
                    float tempDist = Vector3.Distance(go.transform.position, target.transform.position);
                    if (dist > tempDist)
                    {
                        dist = tempDist;
                        targetBone = go;
            ...
            //Debug.LogWarning("Getting New TargetBone : " + targetBone);
        }
       
        //clamp force
        ////Debug.Log("Force Applied =" + force);

        targetBone.GetComponent<Rigidbody>().AddForceAtPosition(force * 0.1f, position,ForceMode.Impulse);
