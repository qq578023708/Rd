...
EDataValidationResult URdExperienceDefinition::IsDataValid(FDataValidationContext& Context) const
{
	...
	if(!GetClass()->IsNative())
	{
		...
		{
			Context.AddError(FText::Format(LOCTEXT("ExperienceInheritenceIsUnsupported","Blueprint subclass of blueprint experiences is not currently supported(use composition via ActionSets instead).Parent class was {0} but should be {1}."),
